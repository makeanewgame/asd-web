      ...
      <div className="flex justify-start mb-8">
        <div className="overflow-x-auto -mx-4 px-4 scrollbar-hide">
          <Tabs
            ...
            variant="underlined"
            classNames={{
              // tabList'ı nowrap yapan ve inline-flex kullanan sınıf
              tabList: `
        inline-flex flex-nowrap gap-3 sm:gap-6 
        rounded-none p-0
        min-w-max
      `,
              cursor: "w-full bg-primary",
              // her tab shrink olmasın
              tab: `
        flex-none max-w-fit px-3 sm:px-4 h-10 sm:h-12
      `,
              tabContent: `
        group-data-[selected=true]:text-[#ADD2FF]
      ...
      `,
          ...
          >
            {projectTypes.map((type, idx) => (
              <Tab
                key={typeof type === "string" ? type : `${String(type)}-${idx}`}
                title={<div>{type}</div>}
              />
            ))}
          </Tabs>
